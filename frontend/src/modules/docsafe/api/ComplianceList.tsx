...
export const ComplianceList: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (editingItemId) {
      success = await updateComplianceItem(editingItemId, itemPayload);
    } else {
      success = await createComplianceItem(itemPayload);
    }
  ...
  const handleQuickStatusChange = async (id: string, newStatus: ComplianceStatus) => {
    const item = complianceItems.find(c => c.id === id);
    if (!item) return;

    await updateComplianceItem(id, {
      name: item.name,
